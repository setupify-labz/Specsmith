...
describe('RetailerLinkCta — fallback-search state', () => {
  const link: RetailerLink = { state: 'fallback-search', href: 'https://www.amazon.com/s?k=RTX+4090&tag=specsmithpc-20' };

  it('says "Search <Retailer>", never an exact/live implication', () => {
    render(<RetailerLinkCta retailer="Amazon" partName="RTX 4090" link={link} variant="pill" sponsored accentColor="#000" />);
  ...
  it('carries the state and an accessible explanation to confirm the model', () => {
    render(<RetailerLinkCta retailer="Newegg" partName="RTX 4090" link={link} variant="text" sponsored={false} accentColor="#000" />);
    const anchor = screen.getByRole('link');
  ...
  it('is sponsored only when the caller says the destination carries attribution', () => {
    render(<RetailerLinkCta retailer="Amazon" partName="RTX 4090" link={link} variant="pill" sponsored accentColor="#000" />);
...
describe('RetailerLinkCta — exact state', () => {
  const link: RetailerLink = { state: 'exact', href: 'https://www.newegg.com/p/N82E16819113476?item=N82E16819113476' };

  it('says "View at <Retailer>" and marks the destination as an exact product page', () => {
    render(<RetailerLinkCta retailer="Newegg" partName="AMD Ryzen 5 5600" link={link} variant="pill" sponsored accentColor="#000" />);
    const anchor = screen.getByRole('link');
...
describe('RetailerLinkCta — unavailable state', () => {
  const link: RetailerLink = { state: 'unavailable', href: null };

  it('renders no link at all — never a clickable destination with nowhere to go', () => {
    render(<RetailerLinkCta retailer="Amazon" partName="" link={link} variant="pill" sponsored accentColor="#000" />);
