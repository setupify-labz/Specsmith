...
//
// A platform may be refused or adapted only by an ESTABLISHED, source-bound
// capability. Unknown is not false, a lower bound is not an upper bound, and a
// capability value whose fact id cannot be resolved in this snapshot is not
// evidence.
...
export type ConflictResolution = "restructure" | "layer-information" | "change-format" | "choose-another-platform" | "refuse-adaptation";
...
export interface RefusalReason { readonly code: string; readonly explanation: string; }
...
export interface AccessibilityRequirement { readonly code: string; readonly requirement: string; readonly negotiable: false; }
export interface CtaGuidance { readonly include: boolean; readonly treatment: string; readonly reason: string; }
...
function sourceBound<T>(capability: Capability<T>, facts: readonly PlatformFact[]): boolean {
  return capability.value !== null
    && capability.factId !== null
    && facts.some((fact) => fact.factId === capability.factId);
}

function hasBindingValue<T>(capability: Capability<T>, facts: readonly PlatformFact[]): capability is Capability<T> & { readonly value: T } {
  return sourceBound(capability, facts) && statusIsBinding(capability.status);
}

function hasUsableValue<T>(capability: Capability<T>, facts: readonly PlatformFact[]): capability is Capability<T> & { readonly value: T } {
  return sourceBound(capability, facts) && statusIsUsable(capability.status) && capability.status !== "hypothesis";
}
...
export function assessAudiencePlatformFit(input: FitInput): AudiencePlatformFit {
  ...
  if (hasBindingValue(capability.media.orientation, facts) && capability.media.orientation.value !== "vertical") {
    refusals.push({
      code: "media-incompatible",
      explanation: `${snapshot.platform} has a source-bound orientation constraint of ${capability.media.orientation.value}, incompatible with SpecSmith's vertical short-form output.`,
    });
  }

  const honestySeconds = requiredHonestySeconds(invariant);
  const maxDuration = hasBindingValue(capability.media.maxDurationSeconds, facts)
  ...
  if (maxDuration !== null && honestySeconds > maxDuration) {
    refusals.push({
      ...
      explanation:
        `Required wording and disclosures take about ${honestySeconds.toFixed(1)}s, longer than the source-bound ` +
        `${maxDuration}s maximum. The claim cannot be shortened by deleting a caveat, so this format is refused.`,
    });
  }

  if (hasUsableValue(capability.text.descriptionMaxChars, facts) && invariant.requiredDisclosures.length > 0) {
    ...
    if (disclosureChars > capability.text.descriptionMaxChars.value) {
      conflicts.push({
        ...
        tension: `Required disclosure instructions total ${disclosureChars} characters while the source-bound description limit is ${capability.text.descriptionMaxChars.value}.`,
        resolution: "restructure",
        explanation:
          "Keep the disclosure in the video and use only a text surface with independently established capacity. This is not an absolute refusal because another surface has not been proven absent.",
      });
    }
  }
  ...
  if (audienceFit.explanationDepth === "extensive" && maxDuration !== null) {
    conflicts.push({
      ...
      tension: `This audience needs extensive explanation (${(profile.knowledge.cannotAssume.value ?? []).length} terms cannot be assumed) within a source-bound ${maxDuration}s maximum.`,
      resolution: "layer-information",
      explanation:
        "Layer supporting detail while keeping every claim, caveat and disclosure intact. If the complete truthful message cannot fit after restructuring, choose a longer format instead of deleting evidence.",
    });
  }
  ...
  if (profile.segmentsConflict) {
    conflicts.push({
      ...
      explanation: "Scope this creative to the primary segment and preserve the second as a separate mission candidate instead of pretending one cut can serve incompatible needs.",
    });
  }
  ...
  if (invariant.estimatedClaimIds.length > 0) {
    conflicts.push({
      ...
      resolution: "restructure",
      explanation: "Shorten surrounding copy, not the evidence boundary. Each line stating the estimate keeps its estimate label and required configuration.",
    });
  }
  ...
  if (refusals.length > 0) {
    ...
    verdict = "unknown";
    reasons.push("No external platform facts are established in this snapshot. Unknown capability stays unknown rather than becoming compatibility or incompatibility.");
  } else {
  ...
  if (input.postingTime.state === "unknown") reasons.push("Posting time is unknown and cannot improve or worsen fit.");
  ...
  return {
    ...
    cta: deriveCta(mission, audienceFit, snapshot, facts),
    ...
    unresolvedAssumptions: [
      ...
      ...(!sourceBound(capability.media.orientation, facts) ? ["Accepted platform orientation is unverified or lacks source-bound provenance."] : []),
      ...(!sourceBound(capability.media.maxDurationSeconds, facts) ? ["Maximum platform duration is unverified or lacks source-bound provenance."] : []),
      ...(!sourceBound(capability.interaction.outboundLinkInDescription, facts) ? ["Outbound-link clickability is unverified or lacks source-bound provenance."] : []),
      "No platform ranking or distribution behaviour is assumed.",
    ],
    platformRisks: derivePlatformRisks(snapshot, invariant, facts),
    staleInputs: stale,
  };
}
...
function derivePacing(audienceFit: AudienceFit, profile: AudienceProfile, invariant: TruthInvariant): PacingGuidance {
  ...
  return {
    ...
    isHypothesis: true,
    basis: "A creative hypothesis derived from explanation needs, not a measured platform optimum. MASTER #5 owns performance testing.",
...
function deriveCaptions(audienceFit: AudienceFit): CaptionGuidance {
  ...
  return {
    ...
    basis: "Line length and display time are unmeasured creative guidelines. Burned-in captions are a SpecSmith-owned accessibility requirement because their rendered output can be inspected locally.",
  };
}
...
function deriveCta(
  ...
  if (!audienceFit.ctaAppropriate) return { include: false, treatment: "none", reason: audienceFit.ctaReason };
  ...
  if (hasUsableValue(link, facts) && link.value === true) {
  ...
  } else if (hasUsableValue(link, facts) && link.value === false && hasUsableValue(profile, facts) && profile.value === true) {
    ...
    treatment = `Name the tool and show ${mission.productRoute} on screen. Do not claim the description is clickable or that a profile-link surface exists until either capability is source-bound.`;
  }
  return { include: true, treatment, reason: `${mission.ctaIntent} This is an offer, not manufactured urgency; unknown platform link behaviour remains unknown.` };
}
...
function deriveHookConstraints(invariant: TruthInvariant, profile: AudienceProfile): readonly string[] {
  ...
  if (invariant.estimatedClaimIds.length > 0) constraints.push("If the hook states an estimated figure, its estimate status and required configuration stay in the hook itself.");
...
function derivePlatformRisks(
  ...
  const link = snapshot.capability.interaction.outboundLinkInDescription;
  if (!sourceBound(link, facts)) {
    risks.push("Outbound-link clickability is unknown or lacks source-bound provenance; do not design the CTA around an assumed clickable description.");
  } else if (link.value === false) {
    risks.push("A source-bound fact establishes that description/caption links are not clickable; use only a separately verified route surface.");
  }
  if (invariant.estimatedClaimIds.length > 0) risks.push("Estimated figures must keep their estimate label and configuration visually attached to the figure.");
  if (!sourceBound(snapshot.capability.analytics.availableMetrics, facts)) {
    risks.push("Analytics metric availability is unknown or lacks source-bound provenance; do not promise that a specific post-publication metric will be measurable.");
  }
  return risks;
}
