/*! userbenchmark 2026-01-09 */
function getEBID(a) {
    return document.getElementById(a)
}
function detailsSearchNaviation(a) {
    var b = document.getElementById("navForm:searchInput").value
      , c = "/Search?searchTerm=" + encodeURIComponent(b);
    a = a.replace("/Search?", c),
    window.location = a
}
...
function monkeyPatchAutocomplete() {
    ...
    $.ui.autocomplete.prototype._renderItem = function(a, b) {
        var c = new RegExp(this.term,"i")
          , d = b.label.replace(c, "<span style='font-weight:bold;color:darkorange;'>$&</span>");
        return $("<li></li>").data("item.autocomplete", b).append("<a>" + d + "</a>").appendTo(a)
    }
    ,
    $.ui.autocomplete.prototype._renderMenu = function(a, b) {
        var c = this;
        $.each(b, function(b, d) {
            100 > b && c._renderItemData(a, d)
        })
    }
}
...
function mhasearchcomp(a, b) {
    $(a).autocomplete({
        ...
        select: function(a, b) {
            ...
            $(this).siblings(".acomptarget").click()
        }
    })
}
function mhacompdebug(a, b, c) {
    $(a).autocomplete({
        source: function(a, c) {
            addMessage("search on: '" + a.term + "'<br/>");
            var d = $.ui.autocomplete.escapeRegex(a.term)
              , e = new RegExp(d,"i")
              , f = $.grep(b, function(a) {
                return e.test(a)
            });
            addMessage("Result: " + f.length + " items<br/>"),
            c(f)
        },
        select: function(a, b) {
            "undefined" == typeof b ? addMessage("You selected undef: " + a + "<br/>") : (addMessage("You selected defin: " + b.item.value + "<br/>"),
            addMessage("nexts input: " + $(this).closest(".form-group").next(".form-group").find("input").attr("id") + "<br/>"))
        },
        minLength: 1,
        autoFocus: c,
        delay: 0
...
function MHAjaxStart() {
    jQuery(".ajaxProgress").addClass("ajaxProgressActive")
}
function MHAjaxEnd() {
    jQuery(".ajaxProgress").removeClass("ajaxProgressActive")
}
function MHAjaxLightsOnOffAndScroll(a, b) {
    var c = a.status;
    switch (c) {
    case "begin":
        MHAjaxStart();
...
function sendJSON_FRC(a, b) {
    form = $(a).closest("form"),
    form.attr("action", window.location),
    controller = form.children(".mh-ajax-controller"),
    1 != controller.length ? alert("Logical error. Please contact support.") : (controller.children("input").val(JSON.stringify(b)),
    controller.trigger("click"))
}
function guiLogMessage(a) {
    sendJSON({
        fn: "log",
        MSG: a
    })
...
function sendJSON(a) {
    document.getElementById("navForm:ajaxJSON").value = JSON.stringify(a),
    document.getElementById("navForm:sendJSON").click()
}
function onNavigateToHotPrice(a) {
    var b = 1 == $(a).parent().parent().siblings("div").children("div").children(":contains('Newegg')").length
      , c = !0;
    if (usernotchosen && (2 > navcount || b && 3 > navcount)) {
        switch (navcount) {
        case 0:
            window.open(a.href, "_blank"),
            onValidateChecksAndSetMerchants($(a).parent().parent().siblings("div").children("div").children(":contains('Ebay')"), !0),
            onValidateChecksAndSetMerchants($(a).parent().parent().siblings("div").children("div").children(":contains('Amazon')"), !0),
            c = !1;
            break;
        case 1:
            window.open(a.href, "_blank"),
            b ? (onValidateChecksAndSetMerchants($(a).parent().parent().siblings("div").children("div").children(":contains('Newegg')"), !0),
            onValidateChecksAndSetMerchants($(a).parent().parent().siblings("div").children("div").children(":contains('Ebay')"), !0)) : onValidateChecksAndSetMerchants($(a).parent().parent().siblings("div").children("div").children(":contains('Amazon')"), !0),
            c = !1;
            break;
        case 2:
            window.open(a.href, "_blank"),
...
function onValidateChecksAndSetMerchants(a, b) {
    $(a).toggleClass("extramutedtext").children("i").toggleClass("fa-check-square-o fa-square-o");
    var c = $(a).parent().children().children(".fa-check-square-o").parent(":contains('Amazon')").length
      , d = $(a).parent().children().children(".fa-check-square-o").parent(":contains('Ebay')").length
      , e = $(a).parent().children().children(".fa-check-square-o").parent(":contains('Newegg')").length
      , f = (c ? "Amazon" : "") + (d ? "Ebay" : "") + (e ? "Newegg" : "");
    if ($(a).parent().parent().siblings("div").children("div").children("a").each(function() {
        var a = this.href.substr(0, this.href.indexOf("Go/Hot") + 9)
          , b = this.href.replace(a, "");
        b = b.substr(b.indexOf("/"), b.length),
        this.href = a + f + b
    }),
    1 != b) {
        var g = a.className.match(/(btn-\w+)/)[1]
          , h = "";
        "btn-default" == g && (h = "ambertext"),
        0 == $(a).parent().children().children(".fa-check-square-o").length ? $(a).parent().parent().siblings("div").children("div").children("a").addClass("disabled").removeClass(h).addClass("extramutedtext") : $(a).parent().parent().siblings("div").children("div").children("a").removeClass("disabled").addClass(h).removeClass("extramutedtext"),
        usernotchosen = !1;
        var i = g + "-3d"
          , j = g;
        $(a).parent().children("a").addClass(i).removeClass(j),
        guiLogMessage("HOTCHOOSE_" + g + "_" + f)
    }
}
function popRatingDialog(a) {
    var b = {
        fn: "pop",
        popAction: "popRat"
    };
    for (var c in a)
        b[c] = a[c];
    document.getElementById("ratingDialogForm:hinputid").value = JSON.stringify(b),
    document.getElementById("ratingDialogForm:ratdidpopup").click()
}
function ratingDialogAction(a, b) {
    $("#myRatingModal").modal("hide"),
    wr = $(".mh-ajaxpager"),
    anumpp = wr.attr("data-ajpnpp"),
    acpage = wr.attr("data-ajpcp");
    var c = {
        fn: "spr",
        anumpp: anumpp,
        acpage: acpage
...
function ratingDialogCommentExpand(a) {
    var b = $(a)
      , c = b.siblings("textarea");
    83 == c.outerHeight() ? (c.animate({
        height: 520
    }),
    b.children("i").switchClass("fa-arrow-down", "fa-arrow-up")) : (c.animate({
        height: 83
    }),
    b.children("i").switchClass("fa-arrow-up", "fa-arrow-down"))
}
function ratingDialogInitStars() {
    $("#ratingDialogForm\\:mh-star-rating").rating({
        filledStar: "&#x2605;",
        emptyStar: "&#x2606;",
        showClear: !1,
        min: -1,
        ...
        starCaptions: function(a) {
            return 2 >= a ? "I hate it" : 4 >= a ? "I don't like it" : 6 >= a ? "It's okay" : 8 >= a ? "I like it" : 10 >= a ? "I love it" : "Your reason."
        },
        starCaptionClasses: function(a) {
            return a > 0 ? "none" : void 0
        }
    }).on("rating.change", function() {
        $(this).parent("").removeClass("mh-star-hovering"),
        g_mhStarRemovedOn = $(this).val()
    }).on("rating.hover", function(a, b) {
        g_mhStarRemovedOn != b && $(this).parent("").addClass("mh-star-hovering")
    }).on("rating.hoverleave", function() {
        $(this).parent("").removeClass("mh-star-hovering")
    })
}
function setClipboardViaTextArea(a) {
    var b = document.createElement("textarea");
    b.style = "position: absolute; left: -1000px; top: -1000px",
    b.value = a,
...
function buildDynaLinksAndClickHandlersToTrackedFooterFaq() {
    var a = 0;
    $(".tf-pthumb-capt").each(function() {
        const b = 4;
        var c, d = $(this);
        do
            c = 1 + Math.floor(Math.random() * b);
        while (c === a);
        a = c;
        var e = "#tff_" + c;
        const f = "<i class='fa fa-chevron-right ambertext'></i>";
        d.append(f + $("#tffaq a[href=" + e + "]").text()),
        d.on("click", function() {
            history.pushState(null, null, "#tffaq_anchor"),
            $("html,body").animate({
                scrollTop: $("#tffaq_anchor").offset().top
            }, 300, "linear"),
            setTimeout(function() {
                guiLogMessage("NAV_TF_PTHUMB_" + c),
                $("#tffaq a[href=" + e + "]").focus(),
                $(e).collapse("show")
            }, 800)
        })
    })
}
function myPageIsLoaded() {
    $("body").on("mousedown", "a.navtrack", function(a) {
        2 == a.which && guiLogMessage($(this).data("navtrack"))
    }),
    $("body").on("click", "a.navtrack", function(a) {
        var b = $(this).attr("href")
          , c = !1;
        switch (a.which) {
        case 1:
            guiLogMessage($(this).data("navtrack")),
            a.metaKey || a.ctrlKey || (c = !0)
        }
        if (c) {
    ...
    $(function() {
        $("img.lazynonseq").lazyload({
            failure_limit: 100,
            threshold: 400
        })
    }),
    monkeyPatchAutocomplete(),
    $(".mhautoselonfocus").focus(function() {
        $(this).select()
    }).mouseup(function(a) {
        a.preventDefault()
    }),
    $(".mhautofocus").focus(),
    $(".autoCountUp").each(function() {
        var a = $(this);
        jQuery({
            Counter: 0
        }).animate({
            Counter: a.text().replace(/,/g, "")
        }, {
            duration: 1400,
            easing: "easeOutExpo",
            step: function() {
                a.text(Math.ceil(this.Counter).toLocaleString("en"))
            }
        })
    });
    var a = function(a) {
        "event" == a.type ? "begin" == a.status ? (G_AJAXACTIVE = !0,
        (-1 != a.source.id.indexOf("Sorter") || -1 != a.source.id.indexOf("playForm")) && MHAjaxStart()) : "success" == a.status && (G_AJAXACTIVE = !1,
        -1 == a.source.id.indexOf("navForm") && ($(function() {
            $("img.lazy").lazyload({
                threshold: 400
            })
        }),
        ...
        $(window).resize()),
        (-1 != a.source.id.indexOf("Sorter") || -1 != a.source.id.indexOf("playForm")) && MHAjaxEnd()) : (G_AJAXACTIVE = !1,
        console.log("Ajax error, dumping error, then reloading page."),
        console.log(a),
    ...
    try {
        jsf.ajax.addOnEvent(a),
        jsf.ajax.addOnError(a)
    } catch (b) {}
    $("body").on("click", ".mh-vote.bglink", function() {
        wr = $(this).closest(".mh-vote-w"),
        pid = wr.attr("data-pid"),
        mid = wr.attr("data-mid"),
    ...
    $("body").on("click", ".auto-copy", function() {
    ...
    $("#tffaq").on("show.bs.collapse", function(a) {
        var b = $(a.target).attr("data-tffkey");
        guiLogMessage("NAV_TF_" + b),
        $(a.target).siblings("a").find("i").removeClass("fa-chevron-right").addClass("fa-chevron-down"),
        $(a.currentTarget).find(".collapse.in").collapse("hide")
    }),
    $("#tffaq").on("hide.bs.collapse", function(a) {
        $(a.target).siblings("a").find("i").removeClass("fa-chevron-down").addClass("fa-chevron-right")
    })
}
!function(a) {
    ...
    a.widget.extend = function(b) {
        for (var c, d, f = e.call(arguments, 1), g = 0, h = f.length; h > g; g++)
            for (c in f[g])
                d = f[g][c],
                f[g].hasOwnProperty(c) && void 0 !== d && (b[c] = a.isPlainObject(d) ? a.isPlainObject(b[c]) ? a.widget.extend({}, b[c], d) : a.widget.extend({}, d) : d);
        return b
    }
    ,
        ...
        a.fn.position = function(e) {
            ...
            this.each(function() {
                ...
                e.using && (j = function(a) {
                    ...
                    i.important = g(h(b), h(c)) > g(h(d), h(f)) ? "horizontal" : "vertical",
    ...
    a.widget("ui.autocomplete", {
        ...
        _create: function() {
            ...
            this._on(this.menu.element, {
                ...
                menufocus: function(b, c) {
                    ...
                    b.originalEvent && /^mouse/.test(b.originalEvent.type)) ? (this.menu.blur(),
                    ...
                    void (d && a.trim(d).length && (this.liveRegion.children().hide(),
        ...
        _searchTimeout: function(a) {
            ...
            this.searching = this._delay(function() {
                ...
                (!b || b && !c && !d) && (this.selectedItem = null,
                this.search(null, a))
            }, this.options.delay)
        },
        search: function(a, b) {
    ...
    function() {
        ...
        function c(b, c) {
            ...
            for (d in c)
                f = c[d],
                b[d] !== f && (e[d] || (a.fx.step[d] || !isNaN(parseFloat(f))) && (g[d] = f));
            return g
        }
        ...
        a.each(["borderLeftStyle", "borderRightStyle", "borderBottomStyle", "borderTopStyle"], function(b, c) {
            a.fx.step[c] = function(a) {
                ("none" !== a.end && !a.setAttr || 1 === a.pos && !a.setAttr) && (h.style(a.elem, c, a.end),
                a.setAttr = !0)
            }
        }),
        a.fn.addBack || (a.fn.addBack = function(a) {
    ...
    function() {
        ...
        a.extend(a.effects, {
            ...
            save: function(a, b) {
                for (var c = 0; c < b.length; c++)
                    null !== b[c] && a.data(g + b[c], a[0].style[b[c]])
            },
            restore: function(a, b) {
                var c, d;
                for (d = 0; d < b.length; d++)
                    null !== b[d] && (c = a.data(g + b[d]),
                    void 0 === c && (c = ""),
                    a.css(b[d], c))
            },
    ...
    function(a) {
        function e(b, c) {
            ...
            if (this.sliderElem)
                ...
                if (w && this.options.labelledby[0] && m.setAttribute("aria-labelledby", this.options.labelledby[0]),
                w && this.options.labelledby[1] && n.setAttribute("aria-labelledby", this.options.labelledby[1]),
                !w && this.options.labelledby && (m.setAttribute("aria-labelledby", this.options.labelledby),
            ...
            this._state.value = this.options.value) : this._state.value = this.options.range ? [this.options.value, this.options.max] : this.options.value,
          ...
          , g = {
            ...
            logarithmic: {
                toValue: function(a) {
                    ...
                    return Math.round(d) === this.options.max ? this.options.max : (d = this.options.min + Math.round((d - this.options.min) / this.options.step) * this.options.step,
...
function(a) {
    function b() {
        var b = a('<style id="__dragtable_disable_text_selection__" type="text/css">body { -ms-user-select:none;-moz-user-select:-moz-none;-khtml-user-select:none;-webkit-user-select:none;user-select:none; }</style>');
        a(document.head).append(b),
        a(document.body).attr("onselectstart", "return false;").attr("unselectable", "on"),
        window.getSelection ? window.getSelection().removeAllRanges() : document.selection.empty()
    }
    function c() {
        a("#__dragtable_disable_text_selection__").remove(),
        e ? a(document.body).attr("onselectstart", e) : a(document.body).removeAttr("onselectstart"),
        f ? a(document.body).attr("unselectable", f) : a(document.body).removeAttr("unselectable")
    }
    function d(a, b) {
        var c = a.parentNode
    ...
    a.widget("akottr.dragtable", {
        ...
        persistState: function() {
            var b = this;
            this.originalTable.el.find("th").each(function(a) {
                "" !== this.id && (b.originalTable.sortOrder[this.id] = a)
            }),
            a.ajax({
        ...
        _restoreState: function(b) {
            for (var c in b)
                this.originalTable.startIndex = a("#" + c).closest("th").prevAll().length + 1,
                this.originalTable.endIndex = parseInt(b[c], 10) + 1,
                this._bubbleCols()
        },
        _bubbleCols: function() {
            var a, b, c, e, f = this.originalTable.startIndex, g = this.originalTable.endIndex, h = this.originalTable.el.children();
            if (this.options.excludeFooter && (h = h.not("tfoot")),
            g > f)
                for (a = f; g > a; a++)
                    for (c = h.find("> tr > td:nth-child(" + a + ")").add(h.find("> tr > th:nth-child(" + a + ")")),
                    e = h.find("> tr > td:nth-child(" + (a + 1) + ")").add(h.find("> tr > th:nth-child(" + (a + 1) + ")")),
                    b = 0; b < c.length; b++)
                        d(c[b], e[b]);
            else
                for (a = f; a > g; a--)
        ...
        _rearrangeTableBackroundProcessing: function() {
            ...
            return function() {
                b._bubbleCols(),
                b.options.beforeStop(b.originalTable),
                b.sortableTable.el.remove(),
                c(),
                null !== b.options.persistState && (a.isFunction(b.options.persistState) ? b.options.persistState(b.originalTable) : b.persistState())
            }
        },
        _rearrangeTable: function() {
            ...
            return function() {
                ...
                a.originalTable.endIndex = a.sortableTable.movingRow.prevAll().length + 1,
                setTimeout(a._rearrangeTableBackroundProcessing(), 50)
            }
        },
        _generateSortable: function(c) {
            !c.cancelBubble && (c.cancelBubble = !0);
            for (var d = this, e = this.originalTable.el[0].attributes, f = "", g = 0; g < e.length; g++)
                e[g].nodeValue && "id" != e[g].nodeName && "width" != e[g].nodeName && (f += e[g].nodeName + '="' + e[g].nodeValue + '" ');
            var h = []
              , i = [];
            this.originalTable.el.find("tr").slice(0, this.options.maxMovingRows).each(function() {
                for (var b = this.attributes, c = "", d = 0; d < b.length; d++)
                    b[d].nodeValue && "id" != b[d].nodeName && (c += " " + b[d].nodeName + '="' + b[d].nodeValue + '"');
                h.push(c),
                i.push(a(this).height())
            });
            var j = []
              , k = 0
              , l = d.originalTable.el.children();
            if (this.options.excludeFooter && (l = l.not("tfoot")),
            l.find("> tr > th").each(function() {
                var b = a(this).is(":visible") ? a(this).outerWidth() : 0;
                j.push(b),
                k += b
            }),
            d.options.exact) {
                var m = k - d.originalTable.el.outerWidth();
                j[0] -= m
            }
            k += 2;
            var n = '<ul class="dragtable-sortable" style="position:absolute; width:' + k + 'px;">';
            l.find("> tr > th").each(function(b) {
                var c = a(this).is(":visible") ? a(this).outerWidth() : 0;
                n += '<li style="width:' + c + 'px;">',
                n += "<table " + f + ">";
                var e = l.find("> tr > th:nth-child(" + (b + 1) + ")");
                d.options.maxMovingRows > 1 && (e = e.add(l.find("> tr > td:nth-child(" + (b + 1) + ")").slice(0, d.options.maxMovingRows - 1))),
                e.each(function(b) {
                    var c = a(this).clone().wrap("<div></div>").parent().html();
                    0 === c.toLowerCase().indexOf("<th") && (n += "<thead>"),
                    n += "<tr " + h[b] + '" style="height:' + i[b] + 'px;">',
                    n += c,
                    0 === c.toLowerCase().indexOf("<th") && (n += "</thead>"),
                    n += "</tr>"
                }),
            ...
            n += "</ul>",
            this.sortableTable.el = this.originalTable.el.before(n).prev(),
            this.sortableTable.el.find("> li > table").each(function(b) {
                a(this).css("width", j[b] + "px")
            }),
            this.sortableTable.selectedHandle = this.sortableTable.el.find("th .dragtable-handle-selected");
            var o = this.options.dragaccept ? "li:has(" + this.options.dragaccept + ")" : "li";
            ...
            this.originalTable.startIndex = a(c.target).closest("th").prevAll().length + 1,
            this.options.beforeMoving(this.originalTable, this.sortableTable),
            this.sortableTable.movingRow = this.sortableTable.el.find("> li:nth-child(" + this.originalTable.startIndex + ")"),
            b(),
            this.sortableTable.movingRow.trigger(a.extend(a.Event(c.type), {
                which: 1,
                clientX: c.clientX,
                clientY: c.clientY,
                pageX: c.pageX,
            ...
            var p = this.sortableTable.el.find(".ui-sortable-placeholder");
            !p.height() <= 0 && p.css("height", this.sortableTable.el.find(".ui-sortable-helper").height()),
            p.html('<div class="outer" style="height:100%;"><div class="inner" style="height:100%;"></div></div>')
        },
        bindTo: {},
        _create: function() {
            this.originalTable = {
            ...
            this.bindTo.find(this.options.dragHandle).length > 0 && (this.bindTo = this.bindTo.find(this.options.dragHandle)),
            null !== this.options.restoreState && (a.isFunction(this.options.restoreState) ? this.options.restoreState(this.originalTable) : this._restoreState(this.options.restoreState));
            var b = this;
            this.bindTo.mousedown(function(c) {
                1 === c.which && b.options.beforeStart(b.originalTable) !== !1 && (clearTimeout(this.downTimer),
                this.downTimer = setTimeout(function() {
                    b.originalTable.selectedHandle = a(this),
                    b.originalTable.selectedHandle.addClass("dragtable-handle-selected"),
                    b._generateSortable(c)
                }, b.options.clickDelay))
            }).mouseup(function() {
    ...
    var e = a(document.body).attr("onselectstart")
      , f = a(document.body).attr("unselectable")
}(jQuery),
function(a) {
    ...
    if (window.Select2 === b) {
        ...
        E = D(Object, {
            ...
            positionDropdown: function() {
                ...
                g.addClass("select2-drop-above")) : (e.top = r,
                e.bottom = "auto",
...
function(a, b) {
    ...
    a.fn.lazyload = function(d) {
        ...
        return d && (void 0 !== d.failurelimit && (d.failure_limit = d.failurelimit,
        delete d.failurelimit),
        void 0 !== d.effectspeed && (d.effect_speed = d.effectspeed,
        delete d.effectspeed),
        a.extend(h, d)),
        f = void 0 === h.container || h.container === b ? c : a(h.container),
        0 === h.event.indexOf("scroll") && f.bind(h.event, function() {
            return e()
        }),
        this.each(function() {
            var b = this
    ...
    !function(a, b) {
        ...
        f = function() {
            function a(a) {
                ...
                a.bind(p, function() {
                    var a;
                    return b.didScroll || l ? void 0 : (b.didScroll = !0,
                    a = function() {
                        return b.doScroll(),
                        b.didScroll = !1
                    }
                ...
                a.bind(o, function() {
                    var a;
                    return b.didResize ? void 0 : (b.didResize = !0,
                    a = function() {
                        return c[t]("refresh"),
                        b.didResize = !1
                    }
        ...
        g = function() {
            ...
            a.getWaypointsByElement = function(a) {
                var b, d;
                return (d = c(a).data(r)) ? (b = c.extend({}, h.horizontal, h.vertical),
                c.map(d, function(a) {
                    return b[a]
                })) : []
            }
        ...
        m = {
            ...
            _invoke: function(a) {
                var b;
                return b = c.extend({}, h.vertical, h.horizontal),
...
function() {
    !function(a, b) {
        ...
        c = function(a, b) {
            return a.wrap(b.wrapper),
            a.parent()
        }
        ,
        a.waypoints("extendFn", "sticky", function(d) {
            var e, f, g;
            return f = a.extend({}, a.fn.waypoint.defaults, b, d),
            e = c(this, f),
            g = f.handler,
            f.handler = function(b) {
                var c, d;
                return c = a(this).children(":first"),
                d = "down" === b || "right" === b,
                c.toggleClass(f.stuckClass, d),
                e.height(d ? c.outerHeight() : ""),
                null != g ? g.call(this, b) : void 0
            }
            ,
            e.waypoint(f),
            this.data("stuckClass", f.stuckClass)
        }),
        a.waypoints("extendFn", "unsticky", function() {
            return this.parent().waypoint("destroy"),
            this.unwrap(),
...
function(a) {
    ...
    l.prototype = {
        constructor: l,
        _parseAttr: function(a, b) {
            var g, h, i, j, k = this, l = k.$element, m = l.attr("type");
            if ("range" === m || "number" === m) {
                switch (h = b[a] || l.data(a) || l.attr(a),
                a) {
                case "min":
                    i = c;
                    break;
        ...
        _setDefault: function(a, b) {
            var c = this;
            f(c[a]) && (c[a] = b)
        },
        _listenClick: function(a, b) {
            return a.stopPropagation(),
            a.preventDefault(),
            a.handled === !0 ? !1 : (b(a),
            void (a.handled = !0))
        },
        _starClick: function(a) {
            var b, c = this;
            c._listenClick(a, function(a) {
                return c.inactive ? !1 : (b = c._getTouchPosition(a),
                c._setStars(b),
                c.$element.trigger("change").trigger("rating.change", [c.$element.val(), c._getCaption()]),
                void (c.starClicked = !0))
            })
        },
        _starMouseMove: function(a) {
            var b, c, d = this;
            !d.hoverEnabled || d.inactive || a && a.isDefaultPrevented() || (d.starClicked = !1,
            b = d._getTouchPosition(a),
            c = d.calculate(b),
            d._toggleHover(c),
            d.$element.trigger("rating.hover", [c.val, c.caption, "stars"]))
        },
        _starMouseLeave: function(a) {
            var b, c = this;
            !c.hoverEnabled || c.inactive || c.starClicked || a && a.isDefaultPrevented() || (b = c.cache,
            c._toggleHover(b),
            c.$element.trigger("rating.hoverleave", ["stars"]))
        },
        _clearClick: function(a) {
            var b = this;
            b._listenClick(a, function() {
                b.inactive || (b.clear(),
                b.clearClicked = !0)
            })
        },
        _clearMouseMove: function(a) {
            var b, c, d, e, f = this;
            !f.hoverEnabled || f.inactive || !f.hoverOnClear || a && a.isDefaultPrevented() || (f.clearClicked = !1,
            b = '<span class="' + f.clearCaptionClass + '">' + f.clearCaption + "</span>",
            c = f.clearValue,
            d = f.getWidthFromValue(c) || 0,
            e = {
                caption: b,
                width: d,
                val: c
            ...
            f._toggleHover(e),
            f.$element.trigger("rating.hover", [c, b, "clear"]))
        },
        _clearMouseLeave: function(a) {
            var b, c = this;
            !c.hoverEnabled || c.inactive || c.clearClicked || !c.hoverOnClear || a && a.isDefaultPrevented() || (b = c.cache,
            c._toggleHover(b),
            c.$element.trigger("rating.hoverleave", ["clear"]))
        },
        _resetForm: function(a) {
            var b = this;
            a && a.isDefaultPrevented() || b.inactive || b.reset()
        },
        _setTouch: function(a, b) {
            var c, d, e, g, h, i, j, k = this, l = "ontouchstart"in window || window.DocumentTouch && document instanceof window.DocumentTouch;
            l && !k.inactive && (c = a.originalEvent,
            d = f(c.touches) ? c.changedTouches : c.touches,
            e = k._getTouchPosition(d[0]),
            b ? (k._setStars(e),
            k.$element.trigger("change").trigger("rating.change", [k.$element.val(), k._getCaption()]),
            k.starClicked = !0) : (g = k.calculate(e),
            h = g.val <= k.clearValue ? k.fetchCaption(k.clearValue) : g.caption,
            i = k.getWidthFromValue(k.clearValue),
            j = g.val <= k.clearValue ? i + "%" : g.width,
            k._setCaption(h),
            k.$filledStars.css("width", j)))
        },
        _initTouch: function(a) {
            var b = this
              , c = "touchend" === a.type;
            b._setTouch(a, c)
        },
        _initSlider: function(a) {
            var b = this;
            f(b.$element.val()) && b.$element.val(0),
            b.initialValue = b.$element.val(),
            b._setDefault("min", b._parseAttr("min", a)),
            b._setDefault("max", b._parseAttr("max", a)),
            b._setDefault("step", b._parseAttr("step", a)),
            (isNaN(b.min) || f(b.min)) && (b.min = c),
            (isNaN(b.max) || f(b.max)) && (b.max = d),
            (isNaN(b.step) || f(b.step) || 0 === b.step) && (b.step = e),
            b.diff = b.max - b.min
        },
        _initHighlight: function(a) {
            var b, c = this, d = c._getCaption();
            a || (a = c.$element.val()),
            b = c.getWidthFromValue(a) + "%",
            c.$filledStars.width(b),
            c.cache = {
                caption: d,
                width: b,
                val: a
        ...
        _getContainerCss: function() {
            var a = this;
            return "rating-container" + g(a.theme, "theme-" + a.theme) + g(a.rtl, "rating-rtl") + g(a.size, "rating-" + a.size) + g(a.animate, "rating-animate") + g(a.disabled || a.readonly, "rating-disabled") + g(a.containerClass, a.containerClass)
        },
        _checkDisabled: function() {
            var a = this
              , b = a.$element
              , c = a.options;
            a.disabled = void 0 === c.disabled ? b.attr("disabled") || !1 : c.disabled,
            a.readonly = void 0 === c.readonly ? b.attr("readonly") || !1 : c.readonly,
        ...
        _addContent: function(a, b) {
            var c = this
              , d = c.$container
              , e = "clear" === a;
            return c.rtl ? e ? d.append(b) : d.prepend(b) : e ? d.prepend(b) : d.append(b)
        },
        _generateRating: function() {
            var b, c, d, e = this, f = e.$element;
            c = e.$container = a(document.createElement("div")).insertBefore(f),
            h(c, e._getContainerCss()),
            e.$rating = b = a(document.createElement("div")).attr("class", "rating").appendTo(c).append(e._getStars("empty")).append(e._getStars("filled")),
            e.$emptyStars = b.find(".empty-stars"),
            e.$filledStars = b.find(".filled-stars"),
            e._renderCaption(),
            e._renderClear(),
            e._initHighlight(),
            c.append(f),
            e.rtl && (d = Math.max(e.$emptyStars.outerWidth(), e.$filledStars.outerWidth()),
            e.$emptyStars.width(d))
        },
        _getCaption: function() {
            var a = this;
            return a.$caption && a.$caption.length ? a.$caption.html() : a.defaultCaption
        },
        _setCaption: function(a) {
            var b = this;
            b.$caption && b.$caption.length && b.$caption.html(a)
        },
        _renderCaption: function() {
            var b, c = this, d = c.$element.val(), e = c.captionElement ? a(c.captionElement) : "";
            if (c.showCaption) {
                if (b = c.fetchCaption(d),
                e && e.length)
                    return h(e, "caption"),
                    e.html(b),
                    void (c.$caption = e);
                c._addContent("caption", '<div class="caption">' + b + "</div>"),
                c.$caption = c.$container.find(".caption")
            }
        },
        _renderClear: function() {
            var b, c = this, d = c.clearElement ? a(c.clearElement) : "";
            if (c.showClear) {
                if (b = c._getClearClass(),
                d.length)
                    return h(d, b),
                    d.attr({
                        title: c.clearButtonTitle
                    }).html(c.clearButton),
                    void (c.$clear = d);
                c._addContent("clear", '<div class="' + b + '" title="' + c.clearButtonTitle + '">' + c.clearButton + "</div>"),
                c.$clear = c.$container.find("." + c.clearButtonBaseClass)
            }
        },
        ...
        _getTouchPosition: function(a) {
            var b = f(a.pageX) ? a.originalEvent.touches[0].pageX : a.pageX;
            return b - this.$rating.offset().left
        },
        _toggleHover: function(a) {
            var b, c, d, e = this;
            a && (e.hoverChangeStars && (b = e.getWidthFromValue(e.clearValue),
            c = a.val <= e.clearValue ? b + "%" : a.width,
        ...
        _init: function(b) {
            var c = this
              , d = c.$element.addClass("hide");
            return c.options = b,
            a.each(b, function(a, b) {
                c[a] = b
            }),
            (c.rtl || "rtl" === d.attr("dir")) && (c.rtl = !0,
            d.attr("dir", "rtl")),
            c.starClicked = !1,
            c.clearClicked = !1,
            c._initSlider(b),
            c._checkDisabled(),
            c.displayOnly && (c.inactive = !0,
            c.showClear = !1,
            c.showCaption = !1),
            c._generateRating(),
            c._listen(),
            d.removeClass("rating-loading")
        },
        _listen: function() {
            var b = this
              , c = b.$element
              ...
              , e = b.$rating
              , f = b.$clear;
            return k(e, "touchstart touchmove touchend", a.proxy(b._initTouch, b)),
            k(e, "click touchstart", a.proxy(b._starClick, b)),
            k(e, "mousemove", a.proxy(b._starMouseMove, b)),
            k(e, "mouseleave", a.proxy(b._starMouseLeave, b)),
            b.showClear && f.length && (k(f, "click touchstart", a.proxy(b._clearClick, b)),
            k(f, "mousemove", a.proxy(b._clearMouseMove, b)),
            k(f, "mouseleave", a.proxy(b._clearMouseLeave, b))),
            d.length && k(d, "reset", a.proxy(b._resetForm, b)),
            c
        },
        _getStars: function(a) {
            var b, c = this, d = '<span class="' + a + '-stars">';
            for (b = 1; b <= c.stars; b++)
                d += '<span class="star">' + c[a + "Star"] + "</span>";
            return d + "</span>"
        },
        _setStars: function(a) {
            var b = this
              , c = arguments.length ? b.calculate(a) : b.calculate()
              , d = b.$element;
            return d.val(c.val),
            b.$filledStars.css("width", c.width),
        ...
        showStars: function(a) {
            var b = this
              , c = parseFloat(a);
            return b.$element.val(isNaN(c) ? b.clearValue : c),
            b._setStars()
        },
        calculate: function(a) {
            var b = this
              , c = f(b.$element.val()) ? 0 : b.$element.val()
        ...
        getValueFromPosition: function(a) {
            var b, c, d = this, e = i(d.step), f = d.$rating.width();
            return c = d.diff * a / (f * d.step),
            c = d.rtl ? Math.floor(c) : Math.ceil(c),
            b = j(parseFloat(d.min + c * d.step), e),
            b = Math.max(Math.min(b, d.max), d.min),
            d.rtl ? d.max - b : b
        },
        getWidthFromValue: function(a) {
            var b, c, d = this, e = d.min, f = d.max, g = d.$emptyStars;
            return !a || e >= a || e === f ? 0 : (c = g.outerWidth(),
            b = c ? g.width() / c : 1,
            a >= f ? 100 : (a - e) * b * 100 / (f - e))
        },
        fetchCaption: function(a) {
            var b, c, d, e, g, h = this, k = parseFloat(a) || h.clearValue, l = h.starCaptions, m = h.starCaptionClasses;
            return k && k !== h.clearValue && (k = j(k, i(h.step))),
            e = "function" == typeof m ? m(k) : m[k],
            d = "function" == typeof l ? l(k) : l[k],
            c = f(d) ? h.defaultCaption.replace(/\{rating}/g, k) : d,
            b = f(e) ? h.clearCaptionClass : e,
            g = k === h.clearValue ? h.clearCaption : c,
            '<span class="' + b + '">' + g + "</span>"
        },
        destroy: function() {
            var b = this
              , c = b.$element;
            return f(b.$container) || b.$container.before(c).remove(),
        ...
        create: function(a) {
            var b = this
              , c = a || b.options || {};
            return b.destroy().rating(c)
        },
        clear: function() {
            var a = this
              , b = '<span class="' + a.clearCaptionClass + '">' + a.clearCaption + "</span>";
            return a.inactive || a._setCaption(b),
            a.showStars(a.clearValue).trigger("change").trigger("rating.clear")
        },
        reset: function() {
            var a = this;
            return a.showStars(a.initialValue).trigger("rating.reset")
        },
        update: function(a) {
            var b = this;
            return arguments.length ? b.showStars(a) : b.$element
        },
        refresh: function(b) {
            var c = this
              , d = c.$element;
            return b ? c.destroy().rating(a.extend(!0, c.options, b)).trigger("rating.refresh") : d
        }
    },
    a.fn.rating = function(b) {
        ...
        switch (c.shift(),
        this.each(function() {
            var e, g = a(this), h = g.data("rating"), i = "object" == typeof b && b, j = i.language || g.data("language") || "en", k = {};
            h || ("en" === j || f(a.fn.ratingLocales[j]) || (k = a.fn.ratingLocales[j]),
            e = a.extend(!0, {}, a.fn.rating.defaults, a.fn.ratingLocales.en, k, i, g.data()),
            h = new l(this,e),
            g.data("rating", h)),
    ...
    a.fn.rating.Constructor = l,
    a(document).ready(function() {
        var b = a("input.rating");
        b.length && b.removeClass("rating-loading").addClass("rating-loading").rating()
    })
});
var mcCoreSelect2Options = {
    formatNoMatches: function(a) {
        return "<a class='btn btn-warning blacktext' href='javascript:void(0)' onclick=\"window.location.href = '/Search?searchTerm=" + encodeURIComponent(a).replace(/'/g, "%27") + '\'">Click to search entire site for <b>"' + a + '"</b></a>'
    },
    formatResult: function(a) {
        return a.t + '<span class="pull-right">' + (a.p ? a.p : "—&nbsp;&nbsp;") + "</span>"
    },
    matcher: function(a, b) {
        var c = a.split(" ")
          , d = b.lastIndexOf(" • ");
        -1 == d && (d = b.length);
        for (var e = b.substring(b.indexOf(". ") + 1, d), f = 0; f < c.length; f++) {
